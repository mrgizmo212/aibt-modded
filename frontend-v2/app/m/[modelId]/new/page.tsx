...
/**
 * Ephemeral Model Conversation Route
 * URL: /m/[modelId]/new
 * Purpose: New model-specific conversation BEFORE first message is sent
 * No database record exists yet (optimistic creation)
 */
export default function NewModelConversationPage() {
  const router = useRouter()
  const params = useParams()
  const { user, loading } = useAuth()
  
  // Extract model ID from URL params
  ...
  const handleChooseBuilder = () => {
    // Redirect to /new with builder open (general builder, not model-specific)
    router.push('/new')
    setTimeout(() => {
